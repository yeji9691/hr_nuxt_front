    ...
    <VDialog v-model="salaryStore().isDialogVisible" max-width="600">
      ...
      <VCard title="월 급여 조회">
        ...
        <VCardText class="d-flex justify-end flex-wrap gap-3 mt-2">
          <VBtn v-if="salaryStore().isDialogBtn === true" @click="deleteMonthSalary">
            삭제
          </VBtn>
          <VBtn v-if="salaryStore().isDialogBtn === true" @click="pauseMonthSalary">
            승인대기
          </VBtn>
          <VBtn v-if="salaryStore().isDialogBtn === true" @click="updateMonthSalary">
            승인
          </VBtn>
          <VBtn v-else @click="salaryStore().SET_DIALOG(false)">
            확인
          </VBtn>
        </VCardText>
      </VCard>
